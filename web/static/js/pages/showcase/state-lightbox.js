...
export function stateLightbox() {
    ...
    return {
        ...
        _similarScanAnimating: false,      // 56c-T3 (Codex P2-A): clip scan preview in-flight guard
        ...
        onClipMagicClick() {
            ...
            if (window.OpenAver && window.OpenAver.prefersReducedMotion) {
                // PRM：跳過光帶（56c-T4 接通後改為直接 this.openSimilarMode()）
                return;
            }
            if (window.GhostFly && typeof window.GhostFly.play56cClipScanPreview === 'function') {
                var self = this;
                self._similarScanAnimating = true;
                window.GhostFly.play56cClipScanPreview(coverEl, function () {
                    self._similarScanAnimating = false;
                    // 56c-T3 placeholder; 56c-T4 將以此 callback 啟動 self.openSimilarMode()
                });
            }
        },
        ...
        closeLightbox() {
            ...
            // 56c-fix-v3: standalone clip-exit（similarExitVideo set，lightboxIndex 仍是進 clip 前舊值）
            // → closingIndex 設 -1 跳過 fly-back，避免新影片封面飛回舊 alice 卡片
            var isClipExitStandalone = !!this.similarExitVideo;
            ...
            // 56c-fix: standalone clip-exit mode — lightbox 關閉時清 similarExitVideo，
